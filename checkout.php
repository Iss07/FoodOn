...
<form action="" method="post">
   ...
   <div class="user-info">
      ...
      <select name="method" class="box" required onchange="handleRedirect(this)">
   <option value="" disabled selected>select payment method --</option>
   <option value="cash on delivery">cash on delivery</option>
   <option value="https://razorpay.typeform.com/to/o2Djjakh#name=xxxxx">credit card</option>
   <!-- <option value="debit card">debit card</option>
   <option value="paytm">paytm</option>
   <option value="https://juspay.io/en-in/blog/how-to-collect-upi-payment-on-your-website-mobile-app">upi</option> -->
</select>
...
<script>
  function handleRedirect(select) {
    ...
    // If the selected value is a URL, redirect
...
document.querySelector('form').addEventListener('submit', function(e) {
  const paymentMethod = document.querySelector('[name="method"]').value;

  if (paymentMethod.startsWith('http')) {
    e.preventDefault(); // Stop form from submitting
    window.open(paymentMethod, '_blank'); // Redirect to payment URL
  }
});
</script>
